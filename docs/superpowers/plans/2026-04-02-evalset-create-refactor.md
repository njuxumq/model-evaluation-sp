# 问题集生成流程重构实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 重写 `evalset-create.md` 流程文档，将"生成完整评测集"简化为"仅生成问题集"

**Architecture:** 直接重写 Skill 流程文档，采用汇总确认式交互设计，三步骤结构

**Tech Stack:** Markdown Skill 文档，JSONL 格式产物

---

## 文件结构

| 文件 | 操作 | 说明 |
|------|------|------|
| `.claude/skills/model-evaluation/processes/evalset-create.md` | 重写 | 主流程文档，完全替换原有内容 |
| `docs/superpowers/specs/2026-04-02-evalset-create-refactor-design.md` | 参考 | 设计文档（已完成，无需修改） |

---

## Task 1: 重写流程文档

**Files:**
- Modify: `.claude/skills/model-evaluation/processes/evalset-create.md`（完整替换）

- [ ] **Step 1: 编写新流程文档头部和触发条件**

替换文件开头至触发条件部分：

```markdown
---
name: evalset-create
description: Use when user has no evaluation dataset and needs AI to generate a question-only dataset from scratch based on evaluation scene and dimensions
---

# 问题集生成流程

本文档定义生成问题集流程，AI 根据评测场景和评测维度从零生成仅包含问题的评测集。

---

## 触发条件

用户无评测集，需 AI 根据已确认的评测场景和评测维度生成问题集。

---

## 目标

生成包含 `question`、`case_id`、`category` 三字段的 JSONL 问题集，供后续评测使用。

---

## 流程步骤概览

```
步骤1：确认生成依据（场景+维度）
步骤2：收集配置并生成（询问 → 确认 → 生成）
步骤3：预览与保存
```
```

- [ ] **Step 2: 编写步骤1内容**

继续编写步骤1详细内容：

```markdown
---

## 步骤1：确认生成依据

**目的**：确保评测场景和评测维度已在前置阶段确认。

**执行逻辑**：

| 状态 | 判断 | 动作 |
|------|------|------|
| 场景+维度均已确认 | 从历史对话获取 | → 进入步骤2 |
| 场景未确认 | 无相关信息 | → 提示用户先完成场景确认 |
| 维度未确认 | 无相关信息 | → 提示用户先完成维度配置 |

**输出变量**：
- `{scene}`：已确认的评测场景描述
- `{dimensions}`：已确认的评测维度列表
```

- [ ] **Step 3: 编写步骤2内容**

继续编写步骤2详细内容（收集配置并生成）：

```markdown
---

## 步骤2：收集配置并生成

**目的**：一次性收集生成配置，确认后立即生成问题集。

**使用工具**：`AskUserQuestion`（多问题并行收集）

### 配置收集项

使用 AskUserQuestion 工具一次性收集以下配置：

| 项目 | 问题提示 | 默认值 | 是否必填 |
|------|----------|--------|----------|
| **数量** | 生成多少条问题？ | 10 | 是 |
| **类型分布** | 问题类型偏好？ | 均衡分布 | 否 |
| **难度分布** | 难度偏好？ | 均衡分布 | 否 |
| **其他选项** | 是否需要更多配置？ | 无 | 否 |

**类型分布选项**：
- 均衡分布（默认）
- 偏重开放式
- 偏重封闭式
- 偏重场景化

**难度分布选项**：
- 均衡分布（默认）
- 偏重简单
- 偏重中等
- 偏重困难

**其他选项展开**：

当用户选择"其他选项"时，展开以下配置：

| 子项 | 问题提示 | 默认值 |
|------|----------|--------|
| 分类标签 | 使用哪些 category 标签？ | 自动生成 |
| 示例问题 | 是否提供参考示例？ | 无 |
| 特殊约束 | 其他约束要求？ | 无 |

### 配置确认

收集配置后，展示配置摘要供用户确认：

```
生成配置：
- 数量：{数量}条
- 类型分布：{类型分布}
- 难度分布：{难度分布}
- 分类标签：{分类标签}
- 示例问题：{示例或"无"}
- 特殊约束：{约束或"无"}

是否按此配置生成？(Y/n/调整)
```

- **Y** → 开始生成问题集
- **n** → 流程终止
- **调整** → 返回配置收集

### 生成问题集

用户确认后，AI 根据配置生成问题集。

**生成要求**：
- 问题覆盖评测维度关注点
- 问题风格与评测场景匹配
- 遵循用户提供的约束条件

**生成方式**：
- 数量 ≤50：主 Agent 直接生成
- 数量 >50：可选 SubAgent 分批生成

**字段格式**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `case_id` | `string` | 是 | 用例唯一标识，格式 `case-{序号}` |
| `question` | `string` | 是 | 问题内容 |
| `category` | `string` | 是 | 分类标签 |
```

- [ ] **Step 4: 编写步骤3内容**

继续编写步骤3详细内容（预览与保存）：

```markdown
---

## 步骤3：预览与保存

**目的**：让用户确认生成的问题集，确认后保存。

**⚠️ 不可跳过**：此步骤必须等待用户确认，不可直接保存。

### 预览内容

展示前 3-5 条问题和 category 分布统计：

```
生成完成，共 {数量} 条问题：

【预览前5条】
1. [知识问答] 请解释什么是机器学习？
2. [推荐建议] 为我推荐一个适合周末短途旅行的目的地。
3. [行程规划] 如何规划一个三口之家的日本七日游？
4. [创意生成] 请写一首关于春天的短诗。
5. [知识问答] Python 中如何实现多线程？

【分布统计】
- 知识问答：3条
- 推荐建议：2条
- 行程规划：2条
- 创意生成：2条
- 其他：1条

是否确认保存？(Y/n/重新生成)
```

### 用户操作

| 操作 | 动作 |
|------|------|
| **Y（确认保存）** | → 保存文件，流程结束 |
| **n（取消）** | → 流程终止 |
| **重新生成** | → 返回步骤2调整配置 |

### 保存路径

```
{work-dir}/.eval/{session-id}/evalset/evalset-prepared.jsonl
```
```

- [ ] **Step 5: 编写产物定义、变量速查、常见错误**

继续编写产物定义和附录部分：

```markdown
---

## 产物定义

### 文件名

`evalset-prepared.jsonl`

### 字段说明

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `case_id` | `string` | 是 | 用例唯一标识，格式 `case-{序号}` |
| `question` | `string` | 是 | 问题内容 |
| `category` | `string` | 是 | 分类标签 |

### 示例

```jsonl
{"case_id": "case-001", "question": "请解释什么是机器学习？", "category": "知识问答"}
{"case_id": "case-002", "question": "为我推荐一个适合周末短途旅行的目的地。", "category": "推荐建议"}
{"case_id": "case-003", "question": "如何规划一个三口之家的日本七日游？", "category": "行程规划"}
{"case_id": "case-004", "question": "请写一首关于春天的短诗。", "category": "创意生成"}
{"case_id": "case-005", "question": "Python 中如何实现多线程？", "category": "知识问答"}
```

---

## 变量速查

| 变量 | 说明 |
|------|------|
| `{work-dir}` | 当前工作目录 |
| `{session-id}` | 会话目录名，格式 `session-{8位字母数字}` |
| `{scene}` | 已确认的评测场景描述 |
| `{dimensions}` | 已确认的评测维度列表 |

---

## 常见错误

| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 问题未覆盖评测维度 | 生成时未参考维度列表 | 检查维度映射，补充缺失维度的问题 |
| category 分布不均衡 | 未按用户配置分布生成 | 重新生成或调整配置 |
| 用户确认被跳过 | 直接保存未预览 | 步骤3标注 `⚠️ 不可跳过`，必须等待确认 |
| JSONL 格式无效 | 跨行 JSON 或字段缺失 | 确保每行独立完整 JSON 对象 |
| case_id 不唯一 | 重复序号 | 确保 case_id 按序递增，无重复 |
```

- [ ] **Step 6: 完整写入文件**

将上述所有内容合并，完整写入 `.claude/skills/model-evaluation/processes/evalset-create.md` 文件。

完整内容见下方：

<details>
<summary>完整文件内容</summary>

```markdown
---
name: evalset-create
description: Use when user has no evaluation dataset and needs AI to generate a question-only dataset from scratch based on evaluation scene and dimensions
---

# 问题集生成流程

本文档定义生成问题集流程，AI 根据评测场景和评测维度从零生成仅包含问题的评测集。

---

## 触发条件

用户无评测集，需 AI 根据已确认的评测场景和评测维度生成问题集。

---

## 目标

生成包含 `question`、`case_id`、`category` 三字段的 JSONL 问题集，供后续评测使用。

---

## 流程步骤概览

```
步骤1：确认生成依据（场景+维度）
步骤2：收集配置并生成（询问 → 确认 → 生成）
步骤3：预览与保存
```

---

## 步骤1：确认生成依据

**目的**：确保评测场景和评测维度已在前置阶段确认。

**执行逻辑**：

| 状态 | 判断 | 动作 |
|------|------|------|
| 场景+维度均已确认 | 从历史对话获取 | → 进入步骤2 |
| 场景未确认 | 无相关信息 | → 提示用户先完成场景确认 |
| 维度未确认 | 无相关信息 | → 提示用户先完成维度配置 |

**输出变量**：
- `{scene}`：已确认的评测场景描述
- `{dimensions}`：已确认的评测维度列表

---

## 步骤2：收集配置并生成

**目的**：一次性收集生成配置，确认后立即生成问题集。

**使用工具**：`AskUserQuestion`（多问题并行收集）

### 配置收集项

使用 AskUserQuestion 工具一次性收集以下配置：

| 项目 | 问题提示 | 默认值 | 是否必填 |
|------|----------|--------|----------|
| **数量** | 生成多少条问题？ | 10 | 是 |
| **类型分布** | 问题类型偏好？ | 均衡分布 | 否 |
| **难度分布** | 难度偏好？ | 均衡分布 | 否 |
| **其他选项** | 是否需要更多配置？ | 无 | 否 |

**类型分布选项**：
- 均衡分布（默认）
- 偏重开放式
- 偏重封闭式
- 偏重场景化

**难度分布选项**：
- 均衡分布（默认）
- 偏重简单
- 偏重中等
- 偏重困难

**其他选项展开**：

当用户选择"其他选项"时，展开以下配置：

| 子项 | 问题提示 | 默认值 |
|------|----------|--------|
| 分类标签 | 使用哪些 category 标签？ | 自动生成 |
| 示例问题 | 是否提供参考示例？ | 无 |
| 特殊约束 | 其他约束要求？ | 无 |

### 配置确认

收集配置后，展示配置摘要供用户确认：

```
生成配置：
- 数量：{数量}条
- 类型分布：{类型分布}
- 难度分布：{难度分布}
- 分类标签：{分类标签}
- 示例问题：{示例或"无"}
- 特殊约束：{约束或"无"}

是否按此配置生成？(Y/n/调整)
```

- **Y** → 开始生成问题集
- **n** → 流程终止
- **调整** → 返回配置收集

### 生成问题集

用户确认后，AI 根据配置生成问题集。

**生成要求**：
- 问题覆盖评测维度关注点
- 问题风格与评测场景匹配
- 遵循用户提供的约束条件

**生成方式**：
- 数量 ≤50：主 Agent 直接生成
- 数量 >50：可选 SubAgent 分批生成

**字段格式**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `case_id` | `string` | 是 | 用例唯一标识，格式 `case-{序号}` |
| `question` | `string` | 是 | 问题内容 |
| `category` | `string` | 是 | 分类标签 |

---

## 步骤3：预览与保存

**目的**：让用户确认生成的问题集，确认后保存。

**⚠️ 不可跳过**：此步骤必须等待用户确认，不可直接保存。

### 预览内容

展示前 3-5 条问题和 category 分布统计：

```
生成完成，共 {数量} 条问题：

【预览前5条】
1. [知识问答] 请解释什么是机器学习？
2. [推荐建议] 为我推荐一个适合周末短途旅行的目的地。
3. [行程规划] 如何规划一个三口之家的日本七日游？
4. [创意生成] 请写一首关于春天的短诗。
5. [知识问答] Python 中如何实现多线程？

【分布统计】
- 知识问答：3条
- 推荐建议：2条
- 行程规划：2条
- 创意生成：2条
- 其他：1条

是否确认保存？(Y/n/重新生成)
```

### 用户操作

| 操作 | 动作 |
|------|------|
| **Y（确认保存）** | → 保存文件，流程结束 |
| **n（取消）** | → 流程终止 |
| **重新生成** | → 返回步骤2调整配置 |

### 保存路径

```
{work-dir}/.eval/{session-id}/evalset/evalset-prepared.jsonl
```

---

## 产物定义

### 文件名

`evalset-prepared.jsonl`

### 字段说明

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `case_id` | `string` | 是 | 用例唯一标识，格式 `case-{序号}` |
| `question` | `string` | 是 | 问题内容 |
| `category` | `string` | 是 | 分类标签 |

### 示例

```jsonl
{"case_id": "case-001", "question": "请解释什么是机器学习？", "category": "知识问答"}
{"case_id": "case-002", "question": "为我推荐一个适合周末短途旅行的目的地。", "category": "推荐建议"}
{"case_id": "case-003", "question": "如何规划一个三口之家的日本七日游？", "category": "行程规划"}
{"case_id": "case-004", "question": "请写一首关于春天的短诗。", "category": "创意生成"}
{"case_id": "case-005", "question": "Python 中如何实现多线程？", "category": "知识问答"}
```

---

## 变量速查

| 变量 | 说明 |
|------|------|
| `{work-dir}` | 当前工作目录 |
| `{session-id}` | 会话目录名，格式 `session-{8位字母数字}` |
| `{scene}` | 已确认的评测场景描述 |
| `{dimensions}` | 已确认的评测维度列表 |

---

## 常见错误

| 错误 | 原因 | 解决方案 |
|------|------|----------|
| 问题未覆盖评测维度 | 生成时未参考维度列表 | 检查维度映射，补充缺失维度的问题 |
| category 分布不均衡 | 未按用户配置分布生成 | 重新生成或调整配置 |
| 用户确认被跳过 | 直接保存未预览 | 步骤3标注 `⚠️ 不可跳过`，必须等待确认 |
| JSONL 格式无效 | 跨行 JSON 或字段缺失 | 确保每行独立完整 JSON 对象 |
| case_id 不唯一 | 重复序号 | 确保 case_id 按序递增，无重复 |
```

</details>

- [ ] **Step 7: 提交变更**

```bash
git add .claude/skills/model-evaluation/processes/evalset-create.md
git commit -m "refactor(evalset-create): 简化流程为仅生成问题集

- 产物字段简化为 question + case_id + category
- 交互方式改为汇总确认式（一次性收集配置）
- 步骤数从5步简化为3步
- 移除评测模式判断和模型信息收集

Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com>"
```